...
ActiveSocket allows you to easily create fast services over TCP sockets.  ActiveSocket::Base extends ActiveRecord::Base, and over-rides where necessary so that the actual model (and corresponding database) reside on the other side of a TCP socket connection.
