...
ActiveSocket allows you to easily create fast services over TCP sockets.  ActiveSocket::Base extends ActiveRecord::Base, and over-rides where necessary so that the actual model (and corresponding database) reside on the "other side":https://github.com/keithmgould/ActiveSocket-Server/tree/master of a TCP socket connection.
...
Client side, at model initialization (which is app initialization in Rails production), ActiveSocket models hit the "AS-server":https://github.com/keithmgould/ActiveSocket-Server/tree/master to determine the model's structure.  This means your ActiveSocket models will not only have proper columns defined (to assist with AR goodies), but any custom methods defined on your model server-side will be identified locally and properly handled (see example below.)
...
Currently, both client plugin and "server":https://github.com/keithmgould/ActiveSocket-Server/tree/master are under 200 LoC.
...
* All you need to do on the "server":https://github.com/keithmgould/ActiveSocket-Server/tree/master is include environment, the library and declare the port:
